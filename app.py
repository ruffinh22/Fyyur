...
def edit_venue_submission(venue_id):
    ...
    form = VenueForm(obj=venue)  # Pré-remplit le formulaire avec les données de l'objet Venue

    if request.method == 'POST' and form.validate():
        # Mettre à jour les données de l'objet Venue avec celles du formulaire soumis
        form.populate_obj(venue)

        try:
            db.session.commit()
            flash(f"Le lieu {venue.name} a été mis à jour avec succès!")
        except SQLAlchemyError as e:
            db.session.rollback()
            flash(f"Erreur lors de la mise à jour du lieu {venue.name}.")
            app.logger.error(f"Erreur lors de la mise à jour du lieu {venue.name}: {str(e)}")
        finally:
            db.session.close()

        return redirect(url_for('show_venue', venue_id=venue_id))

    # Si la méthode est GET ou si la validation du formulaire a échoué, afficher le formulaire
    return render_template('forms/edit_venue.html', form=form, venue=venue)
